...
export const IMAGE_BLOT_CONTEXT_MENU_WINDOW_KEY = 'openImageBlotContextMenu';

@Injectable({ providedIn: 'root' })
export class ImageBlotContextMenuService {
	...
	open(event: MouseEvent, data?: any, callback: CallbackFunction | null = null) {
		...
		const portal = new ComponentPortal(ImageMenuComponent);
	...
	setContextMenuInWindow() {
		...
		if ((window as any)[IMAGE_BLOT_CONTEXT_MENU_WINDOW_KEY]) {
			return;
		}

		// Expose a global function to open the Angular context menu.
		(window as any)[IMAGE_BLOT_CONTEXT_MENU_WINDOW_KEY] = (event: MouseEvent, data?: any, callback: CallbackFunction | null = null) => {
