...
const TOOLBAR_CONFIG: ToolbarConfig = [[{ header: ['1', '2', '3', false] }], ['bold', 'italic', 'underline', 'link'], [{ list: 'ordered' }, { list: 'bullet' }], [{ align: ALIGNS }], ['clean']];
...
class DefaultTooltip extends BaseTooltip {
...
export class DefaultTheme extends BaseTheme {
	constructor(quill: Quill, options: ThemeOptions) {
		console.log('Building DefaultTheme');
	...
	extendToolbar(toolbar: Toolbar) {
		...
		if (toolbar.container != null) {
			...
			this.tooltip = new DefaultTooltip(this.quill, this.options.bounds);
...
DefaultTheme.DEFAULTS = merge({}, BaseTheme.DEFAULTS, {
	modules: {
		toolbar: {
			handlers: {
				link(value: string) {
