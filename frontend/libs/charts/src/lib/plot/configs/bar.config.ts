...
function isBarTooltipParam(p: unknown): p is BarTooltipParam {
	return p !== null && typeof p === 'object' && Object.prototype.hasOwnProperty.call(p, 'seriesName');
}

// ─── Bar chart ────────────────────────────────────────────────────────────────

export function buildBarOption({ series, title, downloadFilename, thresholds, annotations, periods, i18nDownload }: BuildOptionParams): echarts.EChartsOption {
	...
	return {
		...
		tooltip: {
			...
			formatter: (rawParams: unknown): string => {
				...
				const params = paramList.filter(isBarTooltipParam);
